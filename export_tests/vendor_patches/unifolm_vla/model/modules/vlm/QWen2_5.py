...
def _resolve_torch_dtype(dtype_name):
    ...
    raise ValueError(f"Unsupported torch_dtype for Qwen2.5-VL export: {dtype_name}")
...
class _QWen_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        torch_dtype = _resolve_torch_dtype(qwenvl_config.get("torch_dtype", "float32"))
        ...
        self.config = config
        self.enable_mixed_precision_training = bool(torch.cuda.is_available())

    def _autocast_context(self, dtype=torch.bfloat16, enabled=True):
    ...
    def forward(
        ...
        **kwargs,
        ...
        with self._autocast_context(dtype=torch.bfloat16):
            outputs = self.model(
                input_ids=input_ids,
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        with self._autocast_context(dtype=torch.float16, enabled=self.enable_mixed_precision_training):
